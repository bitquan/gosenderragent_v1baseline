...
from backend.agent.core.storage_paths import assistant_notification_log_path, assistant_repo_index_path, assistant_runs_dir, assistant_sandbox_dir
from analyze_dev_assistant_log import LOG_PATH, REPORT_PATH, parse_log, render_report, load_tickets as load_ticket_board
try:
    from solo_dev_assistant_followups import synthesize_followup_bats_core as _synthesize_followup_bats_core
except ImportError:
    try:
        from scripts.solo_dev_assistant_followups import synthesize_followup_bats_core as _synthesize_followup_bats_core
    except ImportError:
        from backend.scripts.solo_dev_assistant_followups import synthesize_followup_bats_core as _synthesize_followup_bats_core
...
LOCAL_CFG_PATH = REPO_ROOT / "dev_assistant.local.yaml"
...
NOTIFICATION_LOG_PATH = assistant_notification_log_path(REPO_ROOT)
REPO_MAP_PATH = assistant_repo_index_path(REPO_ROOT)
SKILLPACKS_PATH = RUNS_DIR / "assistant_skillpacks.json"
SANDBOX_ROOT = assistant_sandbox_dir(REPO_ROOT)
FOLLOWUP_BAT_REPORT_PATH = RUNS_DIR / "assistant_followup_bats.json"
FOLLOWUP_BAT_REPORT_MD_PATH = RUNS_DIR / "assistant_followup_bats.md"
...
ASSISTANT_RUNTIME_HOST_PATH_PREFIXES = (
    "core/",
    "shared-runtime/",
    "shared-ui/",
    "host/",
...
ASSISTANT_RUNTIME_POLICY_HINTS = {
    "approval",
    "approved",
    "cooldown",
    "decision",
...
ASSISTANT_RUNTIME_POLICY_CORE_PATHS = {
    "backend/agent/core/approval.py",
    "backend/agent/core/patch_review.py",
    "backend/agent/core/permissions.py",
    "backend/agent/core/repair_service.py",
    "backend/agent/core/runtime_context.py",
    "backend/agent/core/tool_loop.py",
    "backend/agent/core/validation_service.py",
}

ASSISTANT_RUNTIME_POLICY_TEST_PATHS = {
    "backend/tests/test_agent_platform.py",
    "backend/tests/test_agent_runtime.py",
    "backend/tests/test_runtime_api.py",
    "backend/tests/test_solo_dev_assistant_done_ticket.py",
    "backend/tests/test_solo_dev_assistant_plan_domains.py",
    "backend/tests/test_solo_dev_assistant_sandbox.py",
}

ASSISTANT_RUNTIME_VALIDATION_HINTS = {
    "cooldown",
    "decision",
    "decisions",
    "fingerprint",
...
def _load_root_cfg() -> dict[str, Any]:
    cfg: dict[str, Any] = {}
    cfg.update(_load_yaml_config(CFG_PATH))
    cfg.update(_load_yaml_config(LOCAL_CFG_PATH))
    return cfg
...
def _repo_hint_paths_for_domain(repo_map: dict[str, Any], domain_focus: str, *, desc: str = "", limit: int = 8) -> list[str]:
    ...
    if domain_focus == "assistant_runtime" and not _assistant_runtime_host_scope(domain_terms):
        preferred_paths = [
            path for path in preferred_paths if not any(path.startswith(prefix.rstrip("/")) for prefix in ASSISTANT_RUNTIME_HOST_PATH_PREFIXES)
        ]
    ...
    return _assistant_runtime_postprocess_paths(files, domain_focus=domain_focus, domain_terms=domain_terms, limit=limit)
...
def _assistant_runtime_host_scope(domain_terms: list[str]) -> bool:
    term_set = {str(term).strip().lower() for term in domain_terms if str(term).strip()}
    return bool(term_set & ASSISTANT_RUNTIME_HOST_HINTS)


def _assistant_runtime_policy_scope(domain_terms: list[str]) -> bool:
    term_set = {str(term).strip().lower() for term in domain_terms if str(term).strip()}
    return bool(term_set & ASSISTANT_RUNTIME_POLICY_HINTS)


def _assistant_runtime_validation_scope(domain_terms: list[str]) -> bool:
    term_set = {str(term).strip().lower() for term in domain_terms if str(term).strip()}
    return bool(term_set & ASSISTANT_RUNTIME_VALIDATION_HINTS)


def _assistant_runtime_policy_rank(path: str) -> int:
    normalized = str(path).replace("\\", "/").lower()
    if normalized == "backend/tests/test_runtime_api.py":
        return 36
    if normalized == "backend/tests/test_agent_platform.py":
        return 34
    if normalized == "backend/tests/test_agent_platform.py":
        return 32
    if normalized == "backend/tests/test_solo_dev_assistant_plan_domains.py":
        return 28
    if normalized == "backend/tests/test_solo_dev_assistant_sandbox.py":
        return 24
    if normalized == "backend/tests/test_solo_dev_assistant_done_ticket.py":
        return 22
    if normalized in ASSISTANT_RUNTIME_POLICY_CORE_PATHS:
        return 32
    if normalized.startswith("backend/agent/core/"):
        return 20
    if normalized.startswith("backend/agent/runtime/"):
        return 18
...
def _assistant_runtime_validation_rank(path: str) -> int:
    normalized = str(path).replace("\\", "/").lower()
    if normalized == "backend/tests/test_agent_platform.py":
        return 48
    if normalized == "backend/tests/test_runtime_api.py":
        return 42
    if normalized == "backend/agent/core/validation_service.py":
        return 40
    if normalized == "backend/agent/core/repair_service.py":
        return 38
    if normalized == "backend/tests/test_agent_runtime.py":
        return 30
    if normalized == "backend/tests/test_solo_dev_assistant_plan_domains.py":
        return 24
    if normalized in ASSISTANT_RUNTIME_POLICY_CORE_PATHS:
...
def _assistant_runtime_postprocess_paths(
    paths: list[str],
    *,
    domain_focus: str,
    domain_terms: list[str],
    ...
    if domain_focus != "assistant_runtime":
        return cleaned[:limit] if limit is not None else cleaned

    if not _assistant_runtime_host_scope(domain_terms):
        filtered = [
            path
            for path in cleaned
            if not any(path.startswith(prefix) for prefix in ASSISTANT_RUNTIME_HOST_PATH_PREFIXES)
        ]
        if filtered:
            cleaned = filtered

    if _assistant_runtime_policy_scope(domain_terms):
        indexed = list(enumerate(cleaned))
        if _assistant_runtime_validation_scope(domain_terms):
            indexed.sort(
                key=lambda row: (
                    -_assistant_runtime_validation_rank(row[1]),
                    -_assistant_runtime_policy_rank(row[1]),
                    row[0],
                )
            )
        else:
            indexed.sort(key=lambda row: (-_assistant_runtime_policy_rank(row[1]), row[0]))
        cleaned = [path for _index, path in indexed]
...
def _extract_domain_terms(desc: str, *, prefer_domain: str | None = None) -> list[str]:
    ...
    if domain_focus == "assistant_runtime":
        for token in (
            ...
            if token in raw_tokens and token not in terms:
                terms.append(token)
...
def _file_relevance_score(
    ...
    *,
    ...
    if preferred == "assistant_runtime":
        if any(normalized.startswith(prefix) for prefix in ASSISTANT_RUNTIME_HOST_PATH_PREFIXES):
            score += 18 if _assistant_runtime_host_scope(domain_terms) else -24
        elif normalized.startswith(("backend/agent/core/", "backend/agent/runtime/", "backend/tests/", "backend/scripts/")):
            score += 6
        if _assistant_runtime_policy_scope(domain_terms):
            score += _assistant_runtime_policy_rank(normalized)
        if _assistant_runtime_validation_scope(domain_terms):
            score += _assistant_runtime_validation_rank(normalized)
...
def _maybe_close_bat_ticket(ticket_id: str, *, command: str = "implement", check_results: list[dict[str, Any]], cfg: dict[str, Any]) -> dict[str, Any]:
    if command != "implement":
        return {"updated": False, "reason": "implement-only"}
...
def _update_assistant_dashboard() -> None:
    ...
    blocked_rows = [row for row in recent if str(row.get("blocked_reason") or "").strip()]
    skipped_count = sum(1 for row in recent if row.get("all_checks_passed") is None and not row.get("checks"))
    ...
    top_blocked_reasons = [
        ...
        for reason, count in sorted(blocked_reason_counts.items(), key=lambda item: (-item[1], item[0]))[:5]
    ]
    dashboard = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
    ...
    lines = [
        ...
        f"- Skipped: {dashboard['skipped_count']}",
        "",
        "## Recent Tickets",
    ]
...
def _safe_guidance(blocked_reason: str) -> dict[str, Any]:
    lower = str(blocked_reason or "").strip().lower()
    if not lower:
        return {
            "owner_hint": "operator review",
            "protected_why": "Protected by assistant policy.",
            "recommended_next_actions": [
                "Review the BAT manually before resuming assistant execution.",
            ],
        }
    if any(term in lower for term in ("payment", "wallet", "network pool")):
        return {
            "owner_hint": "backend owner + operator review",
            "protected_why": "Touches payment, wallet, or GoSenderr Network Pool behavior.",
            "recommended_next_actions": [
                "Confirm balance and GoSenderr Network Pool invariants.",
                "Approve the protected BAT only after economic checks are reviewed.",
            ],
        }
    if "migration" in lower:
        return {
            "owner_hint": "backend owner + deploy review",
            "protected_why": "Touches migration-sensitive schema or rollout behavior.",
            "recommended_next_actions": [
                "Review schema safety and rollback path.",
                "Approve only after deployment impact is understood.",
            ],
        }
    if "auth" in lower or "security" in lower:
        return {
            "owner_hint": "security-aware reviewer",
            "protected_why": "Touches auth or security-sensitive behavior.",
            "recommended_next_actions": [
                "Review auth/security impact before approving assistant edits.",
            ],
        }
    return {
        "owner_hint": "operator review",
        "protected_why": "Blocked by protected assistant policy.",
        "recommended_next_actions": [
            "Review the blocked BAT and decide whether to split or approve protected work.",
        ],
    }
...
def _sandbox_settings(args: argparse.Namespace | None = None, cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    resolved_cfg = cfg or _load_root_cfg()
    requested_dir = getattr(args, "sandbox_dir", None) if args else None
    sandbox_dir = requested_dir or resolved_cfg.get("assistant_sandbox_dir") or str(SANDBOX_ROOT)
    max_active = _cfg_int(resolved_cfg, "assistant_sandbox_max_active", 0)
    return {
        "sandbox_dir": str(sandbox_dir),
        "reuse_after_hours": max(1, _cfg_int(resolved_cfg, "assistant_sandbox_reuse_hours", 12)),
        "stale_after_hours": max(1, _cfg_int(resolved_cfg, "assistant_sandbox_stale_hours", 24)),
        "max_active": max(0, max_active),
        "auto_cleanup": _cfg_bool(resolved_cfg, "assistant_sandbox_auto_cleanup", True),
    }
    ...
    *,
    ...
    for path in _list_sandbox_dirs(base_dir):
        ...
        if resolved in preserve:
            continue
        metadata = _load_sandbox_metadata(path)
        if not metadata or str(metadata.get("source_repo") or "") != str(REPO_ROOT):
            continue
        if _sandbox_is_dirty(path) and not allow_dirty:
            continue
        generated_at = _sandbox_generated_at(metadata) or datetime.fromtimestamp(0, tz=timezone.utc)
        candidates.append((generated_at, path))

    allowed_existing = max(0, max_active - max(0, reserve_slots))
    excess = len(candidates) - allowed_existing
    if excess <= 0:
        return []

    removed: list[str] = []
    for _generated_at, path in sorted(candidates, key=lambda item: item[0])[:excess]:
        result = _run_cmd(["git", "worktree", "remove", "--force", str(path)], capture=True)
        if result.returncode == 0:
            removed.append(str(path))
    if removed:
...
def _prepare_sandbox(
    ...
    *,
    ...
    cleaned = _cleanup_stale_sandboxes(base_dir, stale_after_hours=stale_after_hours) if auto_cleanup else []
    reusable = _find_reusable_sandbox(base_dir, ticket_id, reuse_after_hours=reuse_after_hours)
    ...
    trimmed = _cleanup_excess_sandboxes(
        base_dir,
        max_active=max_active,
        reserve_slots=1,
    ) if auto_cleanup else []
    current_active = sum(
        1
        for path in _list_sandbox_dirs(base_dir)
        if str(_load_sandbox_metadata(path).get("source_repo") or "") == str(REPO_ROOT)
    )
    if max_active > 0 and current_active >= max_active:
        return {
            "path": str(base_dir),
            "base_dir": str(base_dir),
            "requested_dir": sandbox_dir,
            "ok": False,
            "stdout": "",
            "stderr": f"sandbox active limit reached: {current_active} >= {max_active}",
            "reused": False,
            "cleaned": cleaned,
            "trimmed": trimmed,
        }
...
def _safe_summary(settings: dict[str, Any], *, plan: TicketPlan, sandbox: dict[str, Any] | None = None, blocked_reason: str = "", created_files: list[str] | None = None) -> dict[str, Any]:
    guidance = _safe_guidance(blocked_reason)
    return {
        ...
        "blocked_reason": blocked_reason,
        "owner_hint": guidance["owner_hint"],
        "protected_why": guidance["protected_why"],
        "recommended_next_actions": guidance["recommended_next_actions"],
...
def discover_related_files(
    ...
    *,
    ...
    allowed_prefixes = ("backend/", "frontend/", "docs/")
    if domain_focus == "assistant_runtime" and _assistant_runtime_host_scope(domain_terms):
        allowed_prefixes = allowed_prefixes + ("tools/",)
    ...
    for _score, file_path in scored:
        ...
        if len(related) >= limit:
            break
    return _assistant_runtime_postprocess_paths(
        related,
        domain_focus=domain_focus,
        domain_terms=domain_terms,
        limit=limit,
...
def build_ticket_plan(
    ...
    *,
    ...
    if domain_focus == "assistant_runtime" and _assistant_runtime_validation_scope(domain_terms):
        ...
        repo_hint_paths = _clean_paths(validation_preferred + repo_hint_paths, limit=max_related_files)
...
# ---- Followup BAT synthesis helpers ----

def _next_bat_ticket_id(board_text: str) -> int:
    """Return the next available BAT ticket ID based on existing board entries."""
    ids = [int(m) for m in re.findall(r"BAT<(\d+)>", board_text)]
    return (max(ids) + 1) if ids else 1


def _followup_board_phrase(summary: str) -> str:
    """Normalize a ticket summary for duplicate detection."""
    return summary.lower().strip().rstrip(".")


def _board_ticket_summary(line: str) -> str:
    """Extract the summary text from a BAT board line (text after last tag bracket)."""
    m = re.search(r".*\]\s*(.+)$", line)
    return m.group(1).strip() if m else line.strip()


def _load_blocked_reason_counts() -> dict[str, int]:
    """Scan run artifacts and sprint summaries for blocked reasons."""
    counts: dict[str, int] = {}
    if not RUNS_DIR.exists():
        return counts
    for pattern in ("BAT*_implement.json", "BAT*_run.json"):
        for artifact in RUNS_DIR.glob(pattern):
            try:
    ...
    for artifact in RUNS_DIR.glob("sprint_*.json"):
        try:
            data = json.loads(artifact.read_text(encoding="utf-8"))
        except Exception:
            continue
        for skip in data.get("blocked_skips") or []:
            m = re.search(r"\((.+)\)$", str(skip))
            if m:
...
def _collect_assistant_followup_counts() -> tuple[dict[str, dict[str, Any]], int]:
    """Scan OPS-tagged run artifacts for recurring test failures and review rejections."""
    counts: dict[str, dict[str, Any]] = {}
    scanned = 0
    if not RUNS_DIR.exists():
        return counts, scanned
    for suffix in ("implement", "run"):
        for artifact in RUNS_DIR.glob(f"BAT*_{suffix}.json"):
            try:
                data = json.loads(artifact.read_text(encoding="utf-8"))
            except Exception:
                continue
            priority_tag = str(data.get("priority_tag") or "").upper()
            if priority_tag != "OPS":
                continue
            scanned += 1
            ...
            for check in data.get("checks") or []:
                if check.get("ok"):
                    continue
                args_list = check.get("args") or []
                cmd = " ".join(str(a) for a in args_list)
                name = str(check.get("name") or "").lower()
                if "packaged" in name or "packaged:smoke" in cmd:
                    key = f"packaged-smoke::{cmd}"
                    kind = "packaged-smoke"
                    summary = f"Repair packaged desktop smoke regression in {cmd}."
                elif "ui-smoke" in name or ("smoke" in name and "packaged" not in name):
                    key = f"ui-smoke::{cmd}"
                    kind = "ui-smoke"
                    summary = f"Repair assistant UI smoke regression in {cmd}."
                elif "test" in name or "test" in cmd:
                    key = f"test::{cmd}"
                    kind = "test"
                    summary = f"Repair assistant test regression in {cmd}."
                else:
                    continue
                ...
                if ticket and ticket not in counts[key]["source_tickets"]:
                    counts[key]["source_tickets"].append(ticket)
            for decision in data.get("review_decisions") or []:
                if str(decision.get("status") or "") == "rejected":
                    path = str(decision.get("path") or "").strip()
                    if path:
                        key = f"review-reject::{path}"
                        if key not in counts:
                            counts[key] = {
                                "signature": key,
                                "kind": "review-reject",
                                "summary": f"Repair assistant review rejection path for {path}.",
                                "occurrences": 0,
                                "source_tickets": [],
                            }
                        counts[key]["occurrences"] += 1
...
def _merge_assistant_followup_signal(
    counts: dict[str, dict[str, Any]],
    signal: dict[str, Any] | None,
) -> None:
    """Merge a followup signal into the running counts dict."""
    if not signal:
        return
    key = str(signal.get("signature") or "")
    if not key:
        return
    if key not in counts:
        counts[key] = dict(signal)
        return
    counts[key]["occurrences"] = int(counts[key].get("occurrences", 0)) + int(signal.get("occurrences", 1))
    existing = list(counts[key].get("source_tickets") or [])
    for t in signal.get("source_tickets") or []:
        if t not in existing:
            existing.append(t)
    counts[key]["source_tickets"] = existing


def _assistant_followup_signal_from_reason(reason: str) -> dict[str, Any] | None:
    """Map a sprint reason string to an assistant followup signal, if recognised."""
    lower = reason.strip().lower()
    if "no tickets matched" in lower or "no eligible" in lower:
        return {
            "signature": "assistant-idle-queue::self-heal-seed",
            "kind": "idle-queue",
            "summary": (
                "Seed assistant self-improvement TODO backlog when no eligible OPS tickets"
                " remain so scheduler self-heal mode keeps generating assistant work."
            ),
            "occurrences": 1,
            "source_tickets": [],
        }
...
def synthesize_followup_bats(
    *,
    ...
    """Scan run artifacts and synthesize new BAT follow-up tickets on the board."""
    return _synthesize_followup_bats_core(
        repo_root=REPO_ROOT,
        runs_dir=RUNS_DIR,
        followup_report_path=FOLLOWUP_BAT_REPORT_PATH,
        followup_report_md_path=FOLLOWUP_BAT_REPORT_MD_PATH,
        min_occurrences=min_occurrences,
        max_new_bats=max_new_bats,
        additional_reasons=additional_reasons,
        assistant_only=assistant_only,
        next_bat_ticket_id_fn=_next_bat_ticket_id,
        followup_board_phrase_fn=_followup_board_phrase,
        board_ticket_summary_fn=_board_ticket_summary,
        load_blocked_reason_counts_fn=_load_blocked_reason_counts,
        collect_assistant_followup_counts_fn=_collect_assistant_followup_counts,
        merge_assistant_followup_signal_fn=_merge_assistant_followup_signal,
        assistant_followup_signal_from_reason_fn=_assistant_followup_signal_from_reason,
    )


def _sprint_safe_guard_reason(row: dict[str, Any], safe_settings: dict[str, Any]) -> str:
    """Lightweight safe mode guard for sprint ticket selection (no full plan required)."""
    if safe_settings.get("allow_protected"):
        return ""
    blocked_tags = safe_settings.get("blocked_tags", set())
    tags = {tag.upper() for tag in row.get("tags", [])}
    if blocked_tags & tags:
        return f"safe mode blocked protected tags: {', '.join(sorted(blocked_tags & tags))}"
    domain = str(row.get("domain_focus", "")).lower()
    if domain in safe_settings.get("blocked_domains", set()):
        return f"safe mode blocked protected domain: {domain}"
    lower = row.get("desc", "").lower()
    matched = [t for t in safe_settings.get("blocked_terms", []) if t in lower]
    if matched:
        return f"safe mode blocked protected terms: {', '.join(matched[:4])}"
    return ""
...
def run_pipeline_command(args: argparse.Namespace) -> int:
    ...
    sandbox_policy = _sandbox_settings(args, cfg)
    ...
    if safe_settings.get("enabled") and safe_settings.get("prepare_sandbox"):
        sandbox_info = _prepare_sandbox(
            ticket_id,
            sandbox_policy["sandbox_dir"],
            reuse_after_hours=sandbox_policy["reuse_after_hours"],
            stale_after_hours=sandbox_policy["stale_after_hours"],
            max_active=sandbox_policy["max_active"],
            auto_cleanup=sandbox_policy["auto_cleanup"],
        )
...
def run_implement_command(args: argparse.Namespace) -> int:
    ...
    if args.profile not in PROFILE_MAP:
        print(f"[WARN] Unsupported profile '{args.profile}' detected. Falling back to 'aiWrite'.")
        args.profile = "aiWrite"
...
def run_sandbox_prepare_command(args: argparse.Namespace) -> int:
    policy = _sandbox_settings(args)
    ticket_id = _normalize_ticket_id(args.ticket) if getattr(args, "ticket", None) else "adhoc"
    result = _prepare_sandbox(
        ticket_id,
        policy["sandbox_dir"],
        reuse_after_hours=policy["reuse_after_hours"],
...
def run_sandbox_status_command(args: argparse.Namespace) -> int:
    policy = _sandbox_settings(args)
    base_dirs = _managed_sandbox_base_dirs(policy["sandbox_dir"])
    rows = _collect_sandbox_status_rows(
        base_dirs,
        reuse_after_hours=policy["reuse_after_hours"],
        stale_after_hours=policy["stale_after_hours"],
    )
...
def run_sprint_command(args: argparse.Namespace) -> int:
    ...
    state = _load_state()
    repo_map = _ensure_repo_map()
    safe_settings = _safe_settings(args, cfg) if getattr(args, "safe_mode", False) else {"enabled": False}
    ...
    for row in rows:
        ...
        if temp_blocked:
            ...
            continue
        if safe_settings.get("enabled"):
            sfg = _sprint_safe_guard_reason(row, safe_settings)
            if sfg:
                blocked_rows.append(f"BAT<{row['ticket_id']}> ({sfg})")
                continue
    ...
    if not selected:
        if cooldown_minutes > 0:
            no_match_reason = f"No tickets matched sprint filters outside cooldown window ({cooldown_minutes} minutes)"
            print(no_match_reason + ".")
        else:
        ...
        if blocked_rows:
            ...
            for row in blocked_rows[:10]:
                print(f"- {row}")
        # Write the sprint summary even when no tickets ran
        ensure_runs_dir()
        _stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        _summary_json = RUNS_DIR / f"sprint_{args.action}_{_stamp}.json"
        _summary_md = RUNS_DIR / f"sprint_{args.action}_{_stamp}.md"
        _synth_report: dict[str, Any] | None = None
        _no_payload = {
            "action": args.action,
            "count_requested": args.count,
            "status_filter": status_filter,
            "require_tag": (args.require_tag.upper() if args.require_tag else None),
        ...
        if getattr(args, "synthesize_followups", False):
            _synth_cfg = cfg
            _is_self = str(_synth_cfg.get("assistant_autonomy_mode") or "").strip().lower() in {"self", "self-improve", "self-only", "assistant"}
            _synth_reasons: list[str] = [no_match_reason + "."]
            for _blocked in blocked_rows:
                _m = re.search(r"\((.+)\)$", str(_blocked))
                if _m:
                    _synth_reasons.append(_m.group(1).strip())
            _synth_report = synthesize_followup_bats(
                min_occurrences=2,
                max_new_bats=3,
                additional_reasons=_synth_reasons or None,
                assistant_only=_is_self,
            )
            if _synth_report.get("inserted"):
                print("Synthesized follow-up BATs: " + ", ".join(r.get("summary", "") for r in _synth_report["inserted"]))
            else:
                print("Synthesized follow-up BATs: none new")
        if _synth_report:
            _no_payload["followup_report"] = {
                "inserted_count": len(_synth_report.get("inserted") or []),
                "inserted_tickets": [str(row.get("ticket") or "") for row in _synth_report.get("inserted") or [] if str(row.get("ticket") or "")],
                "summary": [str(row.get("summary") or "") for row in _synth_report.get("inserted") or [] if str(row.get("summary") or "")],
            }
        _summary_json.write_text(json.dumps(_no_payload, indent=2) + "\n", encoding="utf-8")
        _summary_md.write_text(f"# Sprint Summary ({args.action})\n\n- No tickets ran.\n- Reason: {no_match_reason}\n", encoding="utf-8")
        return 0
    ...
    for row in selected:
        ...
        outcome = {
            ...
            "artifact": _display_path(artifact) if artifact.exists() else "",
            "generated_at": datetime.now(timezone.utc).isoformat(),
        }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sprint_parser.add_argument("--synthesize-followups", action="store_true", help="synthesize follow-up BAT tickets from run artifacts after sprint")
