...
from backend.agent.core.engine_day2_run import (  # noqa: E402
    build_engine_day2_execution,
    render_engine_day2_execution,
    write_engine_day2_execution,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Execute the bounded Day 2 self-improvement stress slice using the current Day 2 bundle.")
    parser.add_argument("--project-root", default=str(REPO_ROOT), help="project root to inspect")
    parser.add_argument("--bundle-path", help="optional path to an existing engine_day2_bundle.json file")
...
def main() -> int:
    ...
    execution = build_engine_day2_execution(
        project_root,
    ...
    written = write_engine_day2_execution(project_root, execution, output_dir=output_dir)

    if args.json:
        print(json.dumps(execution, indent=2))
    else:
        print(render_engine_day2_execution(execution).rstrip())
        print("")
