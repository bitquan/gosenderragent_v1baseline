...
from backend.agent.core.engine_day1_run import (  # noqa: E402
    build_engine_day1_execution,
    render_engine_day1_execution,
    write_engine_day1_execution,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Execute the bounded Day 1 engine testing slice using the current Day 1 bundle.")
    parser.add_argument("--project-root", default=str(REPO_ROOT), help="project root to inspect")
    parser.add_argument("--bundle-path", help="optional path to an existing engine_day1_bundle.json file")
    parser.add_argument("--project-maintenance-root", help="optional project root for project-maintenance execution")
    parser.add_argument("--owner-automation-root", help="optional project root for owner-automation execution")
    ...
    parser.add_argument("--output-dir", help="optional directory for the generated json and markdown execution artifacts")
...
def main() -> int:
    ...
    execution = build_engine_day1_execution(
        project_root,
        bundle_path=Path(args.bundle_path).expanduser().resolve() if args.bundle_path else None,
    ...
    written = write_engine_day1_execution(project_root, execution, output_dir=output_dir)

    if args.json:
        print(json.dumps(execution, indent=2))
    else:
        print(render_engine_day1_execution(execution).rstrip())
        print("")
