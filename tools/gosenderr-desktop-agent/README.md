...
## Run locally
```bash
cd tools/gosenderr-desktop-agent
npm install
npm run start
```

## Keep Desktop + VS Code In Sync
- Source of truth for shared assistant behavior is:
  - `tools/vscode-dev-assistant-extension/core/*`
- Desktop copies that core into `tools/gosenderr-desktop-agent/core` automatically through `prestart`, `presmoke`, `prepack`, and `predist` hooks.
...
- Verify the sync contract and runtime health:

```bash
make assistant-sync-check
```
