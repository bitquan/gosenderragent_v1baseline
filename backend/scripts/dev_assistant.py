...
try:
    ...
    # platform package is not present but backend/scripts is available.
    from backend.scripts.task_classification import classify_ticket, record_history
    from backend.scripts.strategies import get_strategy, Strategy
...
def load_config() -> dict[str, Any]:
    ...
    cfg = {}
    cfg_path = PROJECT_ROOT / "dev_assistant.yaml"
    if cfg_path.exists():
        try:
...
def search_repo(query: str) -> str:
    ...
    try:
        out = subprocess.check_output(["git", "grep", "-n", query], cwd=str(root), encoding="utf-8")
        return out.strip()
    except Exception:
        pass
...
def get_git_status() -> str:
    ...
    root = Path(os.environ.get("PROJECT_ROOT", str(REPO_ROOT)))
    try:
        out = subprocess.check_output(["git", "status", "--short"], cwd=str(root), encoding="utf-8")
...
def get_git_diff() -> str:
    ...
    root = Path(os.environ.get("PROJECT_ROOT", str(REPO_ROOT)))
    try:
        out = subprocess.check_output(["git", "diff"], cwd=str(root), encoding="utf-8")
        return out.strip()
    except Exception as e:
...
def _legacy_build_repo_index() -> dict[str, list[str]]:
    ...
    try:
        path = PROJECT_ROOT / "repo_index.json"
...
def build_repo_index() -> dict[str, list[str]]:
    if _core_build_repo_index is not None:
        return _core_build_repo_index(
            ...
            output_path=Path(PROJECT_ROOT) / "repo_index.json",
        )
...
def load_memory() -> list[dict[str, Any]]:
    ...
    path = PROJECT_ROOT / "dev_assistant_memory.json"
    if path.exists():
        try:
...
def send_notification(url: str, payload: dict[str, Any]) -> None:
    ...
    try:
        ln = PROJECT_ROOT / "notifications.log"
...
def update_dashboard(entry: dict[str, Any]) -> None:
    ...
    if _core_update_dashboard is not None:
        ...
        return
    path = PROJECT_ROOT / ".dev_agent_runs" / "dashboard.json"
...
def main() -> None:
    ...
    index_path = PROJECT_ROOT / "repo_index.json"
    ...
    if args.conversation:
        history_file = PROJECT_ROOT / ".dev_assistant_history.jsonl"
        def load_history():
            hist = []
        ...
        while True:
            ...
            if low.startswith("/files"):
                try:
                    out = subprocess.check_output(["git", "status", "--short"], cwd=str(PROJECT_ROOT), encoding="utf-8")
                    out = out.strip() or "clean"
                except Exception as e:
                    out = f"error: {e}"
    ...
    for raw in raw_ids:
        ...
        ids.append(tid)
    # if self-update requested, perform and exit
    if args.self_update:
    ...
    if _core_should_trigger_training is not None and _core_should_trigger_training(cfg, run_mode, args):
        train_cmd = _core_build_training_command(Path(PROJECT_ROOT), openai=bool(args.openai), fine_tune_model=args.fine_tune_model) if _core_build_training_command is not None else [sys.executable, "backend/scripts/train_dev_assistant.py", "--output", "scripts/dev_assistant_training.jsonl"]
        ...
        if args.train or (auto_train and run_mode in ("autopilot", "batch", "scheduled")):
            train_cmd = [sys.executable, "backend/scripts/train_dev_assistant.py", "--output", "scripts/dev_assistant_training.jsonl"]
            if args.openai:
